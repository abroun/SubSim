...
//! constructor
CameraSceneNodeAnimator::CameraSceneNodeAnimator(gui::ICursorControl* cursor, const core::vector3df& pos, f32 rotate, f32 zoom, f32 translate)
    : CursorControl(cursor), Pos(pos), Zooming(false), Rotating(false), Moving(false),
    Translating(false), ZoomSpeed(zoom), RotateSpeed(rotate), TranslateSpeed(translate),
    CurrentZoom(5.0f), RotX(140.0f), RotY(10.0f), OldCamera(0), MousePos(0.5f, 0.5f)
{
    #ifdef _DEBUG
    setDebugName("CameraSceneNodeAnimator");
    #endif
...
bool CameraSceneNodeAnimator::OnEvent(const SEvent& event)
{
...
void CameraSceneNodeAnimator::animateNode(ISceneNode *node, u32 timeMs)
{
...
bool CameraSceneNodeAnimator::isMouseKeyDown(s32 key)
{
    return MouseKeys[key];
}


void CameraSceneNodeAnimator::allKeysUp()
{
    for (s32 i=0; i<3; ++i)
        MouseKeys[i] = false;
}
...
void CameraSceneNodeAnimator::setRotateSpeed(f32 speed)
{
    RotateSpeed = speed;
}
...
void CameraSceneNodeAnimator::setMoveSpeed(f32 speed)
{
...
//! Sets the zoom speed
void CameraSceneNodeAnimator::setZoomSpeed(f32 speed)
{
    ZoomSpeed = speed;
}
...
f32 CameraSceneNodeAnimator::getRotateSpeed() const
{
    return RotateSpeed;
}
...
f32 CameraSceneNodeAnimator::getMoveSpeed() const
{
...
//! Gets the zoom speed
f32 CameraSceneNodeAnimator::getZoomSpeed() const
{
    return ZoomSpeed;
}

ISceneNodeAnimator* CameraSceneNodeAnimator::createClone(ISceneNode* node, ISceneManager* newManager)
{
    CameraSceneNodeAnimator * newAnimator =
        new CameraSceneNodeAnimator(CursorControl, Pos, RotateSpeed, ZoomSpeed, TranslateSpeed);
    return newAnimator;
}
